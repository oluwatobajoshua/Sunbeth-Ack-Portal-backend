// Basic integration tests using Node's built-in test runner (Node >= 20)
...
before(async () => {
  ...
  // Log server output to aid debugging if start fails
  serverProc.stdout.on('data', (d) => { try { process.stdout.write(String(d)); } catch {} });
  serverProc.stderr.on('data', (d) => { try { process.stderr.write(String(d)); } catch {} });
